...
var CTE_PESO_ESTANDAR=1,CTE_PESO_SENSIBLE=2,CTE_PESO_ESPECIAL=3;
...
for(var i=0; i<categorias.estandar.length;i++){
    cat_proc.push({name:categorias.estandar[i],cat:'std',checked:false});
}
for(var i=0; i<categorias.sensible.length;i++){
    cat_proc.push({name:categorias.sensible[i],cat:'stv',checked:false});
}
for(var i=0; i<categorias.especial.length;i++){
    cat_proc.push({name:categorias.especial[i],cat:'spc',checked:false});
}

cat_proc=cat_proc.sort(function(a, b){return a.name.localeCompare(b.name)});
...
app.controller('RiskCalculatorController', function($scope) {
    ...
    $scope.clickItem=function(item){
        ...
        console.log(item);
        $scope.items.sort(function(a, b){return a.name.localeCompare(b.name)});
        $scope.items_seleccionados.sort(function(a, b){return a.name.localeCompare(b.name)});
